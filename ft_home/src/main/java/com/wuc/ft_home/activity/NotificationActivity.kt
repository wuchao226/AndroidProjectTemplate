...
import com.wuc.ft_home.databinding.ActivityNotificationBinding
...
class NotificationActivity : ToolbarActivity<ActivityNotificationBinding>() {
    ...
    companion object {
        ...
        private const val mStopAction = "com.wuc.stop" // 暂停继续action
        private const val mDoneAction = "com.wuc.done" // 完成action
        private var mFlag = 0
        private var mIsStop = false // 是否在播放 默认未开始
    }

    override fun setToolbar() {
        mToolbar.setTitle(getString(R.string.notification))
    }

    override fun initView(savedInstanceState: Bundle?) {
        // 检查权限（适用于 Android 13 及以上）
    ...
    private fun createReceiver() {
        ...
        // 注册广播
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) { // API 31
